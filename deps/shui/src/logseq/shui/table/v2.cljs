...
(def MAX_WIDTH 40 #_rem) ;; Max width in rem for a single column
...
;; the return vlaues are simply the numbers in the classes.
...
(defn get-view-prop 
  "Get the config for a specified item. Can be overriden in blocks, specified in config, 
...
(defn lsx 
  ...
  ([param step]
   (case step 
     1 ({"bg" "bg-[color:var(--ls-primary-background-color)]"} param)
     2 ({"bg" "bg-[color:var(--ls-secondary-background-color)]"} param) 
     3 ({"bg" "bg-[color:var(--ls-tertiary-background-color)]"} param)
     4 ({"bg" "bg-[color:var(--ls-quaternary-background-color)]"} param)
     5 ({"bg" "bg-[color:var(--ls-quinary-background-color)]"} param) 
     6 ({"bg" "bg-[color:var(--ls-senary-background-color)]"} param)
     7 ({"bg" "bg-[color:var(--ls-border-color)]"
         "border" "border-[color:var(--ls-border-color)]"} param)
     11 ({"text" "text-[color:var(--ls-secondary-text-color)]"} param)
     12 ({"text" "text-[color:var(--ls-primary-text-color)]"} param))))
...
(rum/defc table-cell [handle-cell-width-change cell {:keys [row-index col-index render-cell show-separator? total-cols set-cell-hover cell-focus] :as opts}]
  (let [cell-ref (rum/use-ref nil)
    ...
    [:div {:ref cell-ref
           ...
           :style (cond-> {:max-width (str MAX_WIDTH "rem") :box-sizing :border-box}
...
(rum/defc table-container [{:keys [columns borders? table-overflow? total-table-width gray set-cell-hover]} & children]
...
(rum/defc root
  ...
  (let [;; In order to highlight cells in the same row or column of the hovered cell, 
        ...
        [table-overflow? handle-root-width-change] (use-table-overflow-at-width total-table-width)
